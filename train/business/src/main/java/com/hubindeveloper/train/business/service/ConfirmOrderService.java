...
public class ConfirmOrderService {
    ...
    public void doConfirm(ConfirmOrderDoReq req){
        ...
        // 保存确认订单表。
         ...
         confirmOrder.setTickets(JSON.toJSONString(ticketReqList));
         confirmOrderMapper.insert(confirmOrder);

        // 查出余票记录。
        DailyTrainTicket dailyTrainTicket = dailyTrainTicketService.selectByUnique(date, trainCode, startStation, endStation);

        // 扣减余票数量，判断余票是否足够。
        for(ConfirmOrderTicketReq ticketReq : req.getTickets()){
            reduceTicket(dailyTrainTicket, ticketReq);
        }

        // 选座。
        // 最终选座结果
        List<DailyTrainSeat> finalSeatList = new ArrayList<>();
        ConfirmOrderTicketReq first = ticketReqList.get(0);
        if(StrUtil.isNotBlank(first.getSeat())){
            log.info("本次购票有选座");
            List<SeatColEnum> colEnumList = SeatColEnum.getColsByType(first.getSeatTypeCode());
            ...
            getSeat(finalSeatList,
                    date,
                    trainCode,
                    first.getSeatTypeCode(),
                    first.getSeat().split("")[0], // 从A1得到A
                    offsetList,
    ...
    private static void reduceTicket(DailyTrainTicket dailyTrainTicket, ConfirmOrderTicketReq ticketReq) {
